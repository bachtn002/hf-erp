odoo.define('ev_stock_package.LinesWidget', function (require) {
'use strict';
...
var LinesWidget = Widget.extend({
    template: 'stock_package_transfer_barcode_lines_widget',
    events: {
        'click .o_remove_line': '_onClickRemoveLine',
        ...
        'click .o_get_data_weight': '_onClickGetDataWeight',
    ...
    init: function (parent, page, pageIndex, nbPages) {
        ...
        this.record_id = parent.actionParams.id;
    ...
    addProduct: function (lineDescription, model, doNotClearLineHighlight) {
        ...
        var $line = $(QWeb.render('stock_package_transfer_barcode_lines_template', {
        ...
        $body.prepend($line);
        $line.on('click', '.o_remove_line', this._onClickRemoveLine.bind(this));
    ...
    */
     ...
     */
     _renderLines: function () {
        ...
        // TODO done Render and append the page summary.
        var $header = this.$el.filter('.o_barcode_lines_header');
        var $pageSummary = $(QWeb.render('stock_package_transfer_summary_template', {
            locationName: this.page.warehouse_name,
            locationDestName: this.page.warehouse_dest_name,
        ...
        if (this.page.lines.length) {
            if (this.model === 'stock.transfer'){
                var $lines = $(QWeb.render('stock_transfer_barcode_lines_template', {
                ...
                var $lines = $(QWeb.render('stock_package_transfer_barcode_lines_template', {
                    lines: this.page.lines,
                    // packageLines: this.getPackageLines(this.page.lines),
                    model: this.model,
                    groups: this.groups,
            ...
            $lines.on('click', '.o_remove_line', this._onClickRemoveLine.bind(this));
    ...
    _updateIncrementButtons: function ($line) {
        ...
        //if quantity less than 1 we need to hide -1 button
        const hideRemoveButton = (line.qty_done < 1);
        $line.find('.o_remove_unit') .toggleClass('d-none', hideRemoveButton);
        const hideAddButton = (qtyDone >= line.qty);
        $line.find('.o_add_unit') .toggleClass('d-none', hideAddButton);
        if (line.product_id){
            // remove all button when line is package cover
            if(line.product_id.is_package_cover){
                $line.find('.o_remove_unit') .toggleClass('d-none', true);
                $line.find('.o_add_unit') .toggleClass('d-none', true);
                $line.find('.o_edit') .toggleClass('d-none', true);
            }
        }
        if (line.qty === 0) {
        ...
        if (qtyDone < line.qty) {
            ...
            // if (!(line.product_id.tracking === 'serial' || line.product_id.tracking === 'lot')) {}
        }
        // }
    },
     ...
     */
     _onClickValidatePage: function (ev) {
        ev.stopPropagation();
        // Check package weight input before confirm
        var pack_weight = document.getElementById('package_weight');
        if(pack_weight){
            if(pack_weight.value <= 0){
                return this.do_warn(false, _t('Weight must be than 0'));
            }
            // call rpc to save weight of package then validate them
            // this.trigger_up('update_package_weight')
        }
    ...
    /**
     * Handles the click on the `remove line button`.
     *
     * @private
     * @param {MouseEvent} ev
     */
     _onClickRemoveLine: function (ev) {
         ...
         this.trigger_up('remove_line', {id: id});
    },
    ...
    _onClickEditLine: function (ev) {
        ev.preventDefault();
        ev.stopPropagation();
        var barcode = $(ev.target).parents('.o_barcode_line').data('barcode');
        var is_package_line = false;
        if(barcode){
            if(barcode.toString().slice(0, 4) === 'SPTR') {
                is_package_line = true;
            }
        }
        var id = $(ev.target).parents('.o_barcode_line').data('id');
        this.trigger_up('edit_line', {id: id, is_package_line: is_package_line});
    },
     ...
     * @param {OdooEvent} ev
     */
    async _onClickGetDataWeight() {
        try {
            document.getElementsByClassName('o_get_data_weight')[0].disabled = true;
            document.getElementById('package_weight').select()
            var check_res = 0
            const response = await fetch('http://localhost:5000/data', {'Access-Control-Allow-Origin': '*'});
            check_res = 1
            if (check_res == 1){
                document.getElementsByClassName('o_get_data_weight')[0].disabled = false;
            }
        }
        catch(err) {
            document.getElementsByClassName('o_get_data_weight')[0].disabled = false;
