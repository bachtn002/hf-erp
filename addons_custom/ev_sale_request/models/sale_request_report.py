...
class SaleRequestReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        for obj in lines:
            ...
            format5 = workbook.add_format({'font_size': 10, 'align': 'right', 'bold': True})
            ...
            sheet.write('A10', 'Địa chỉ', format6)
            sheet.write('A11', 'STT', format6)
            sheet.write('B11', 'Mã hàng hóa', format6)
            sheet.write('C11', 'Tên hàng hóa', format6)
            sheet.write('D11', 'Đơn vị tính', format6)
            sheet.write('E11', 'Số lượng yêu cầu', format6)
            sheet.write('F11', 'Số lượng đáp ứng', format6)
            sheet.write('G11', 'Đơn vị cung ứng'
                               '(warehouse/purchase)', format6)
            ...
            format7 = workbook.add_format(
                {'num_format': 'mmm d yyyy hh:mm AM/PM', 'font_size': 10, 'align': 'center', 'bold': False})
            sheet.write('F7', obj.date_request, format7)
            ...
            for sale_request_line in sale_request_lines:
                ...
                sheet.write('D' + str(row_num), sale_request_line.product_uom.name, format2)
                row_num += 1
                stt += 1
