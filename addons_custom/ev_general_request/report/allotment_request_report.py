...
class AllotmentRequestReport(models.AbstractModel):
    _name = 'report.ev_general_request.allotment_request_report'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        try:
            for obj in lines:
                ...
                sheet = workbook.add_worksheet(obj.general_request_id.name)
                ...
                # sheet.insert_image('A1:C3', 'myimage.png', {'image_data': image})
                ...
                warehouse_des_list = []
                for warehouse_des in obj.allotment_request_lines:
                ...
                warehouse_ids = []
                for warehouse in obj.general_request_id.general_warehouse_group_ids.warehouse_ids:
                    if warehouse.id in warehouse_des_list:
                        warehouse_ids.append(warehouse.id)
                warehouse_list = self.env['stock.warehouse'].search([('id', 'in', warehouse_ids)])
                dict_col_warehouse = {}
                ...
                allotment_request_lines = obj.allotment_request_lines
                row_num = 8
                row_num_qty = 7
                arr_default_code = []
                dict_row_default_code_same = {}
                dict_sum_qty_default_code = {}
                for allotment_request_line in allotment_request_lines:
                    if not allotment_request_line.product_id.default_code in arr_default_code:
                        sheet.write('A' + str(row_num), allotment_request_line.product_id.default_code, format2)
                        sheet.write('B' + str(row_num), allotment_request_line.product_id.name, format2)
                        sheet.write('C' + str(row_num), allotment_request_line.product_id.product_tmpl_id.categ_id.name, format2)
                        sheet.write('D' + str(row_num), allotment_request_line.product_uom.name, format2)
                        dict_row_default_code_same[allotment_request_line.product_id.default_code] = row_num_qty
                        col_warehouse = dict_col_warehouse[allotment_request_line.warehoue_des_id.id]
                        if obj.state == 'done':
                            sheet.write(row_num_qty, col_warehouse, allotment_request_line.qty_apply, format2)
                        else:
                            sheet.write(row_num_qty, col_warehouse, allotment_request_line.qty, format2)

                        dict_sum_qty_default_code[
                            allotment_request_line.product_id.default_code] = allotment_request_line.qty
                        warehouse_des_list.append(allotment_request_line.warehoue_des_id.id)
                        row_num += 1
                        row_num_qty += 1
                    else:
                        if obj.state == 'done':
                            row_num_same_defaultcode = dict_row_default_code_same[
                                allotment_request_line.product_id.default_code]
                            col_warehouse = dict_col_warehouse[allotment_request_line.warehoue_des_id.id]
                            sheet.write(row_num_same_defaultcode, col_warehouse, allotment_request_line.qty_apply, format2)
                        else:
                            row_num_same_defaultcode = dict_row_default_code_same[
                                allotment_request_line.product_id.default_code]
                            col_warehouse = dict_col_warehouse[allotment_request_line.warehoue_des_id.id]
                            sheet.write(row_num_same_defaultcode, col_warehouse, allotment_request_line.qty, format2)

                        dict_sum_qty_default_code[
                            allotment_request_line.product_id.default_code] += allotment_request_line.qty

                    arr_default_code.append(allotment_request_line.product_id.default_code)

                arr_default_code_no_duppicate = []
                for allotment_request_line in allotment_request_lines:
                    if allotment_request_line.product_id.default_code not in arr_default_code_no_duppicate:
                        arr_default_code_no_duppicate.append(allotment_request_line.product_id.default_code)
                ...
                for code in arr_default_code_no_duppicate:
                    ...
                    quantity = self.env['stock.quant']._get_available_quantity(product_id,obj.general_request_id.warehouse_id.lot_stock_id)
