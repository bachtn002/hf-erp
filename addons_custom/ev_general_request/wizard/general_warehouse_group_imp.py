...
class GeneralWarehouseGroupIMP(models.TransientModel):
    _name = 'general.warehouse.group.import'

    upload_file = fields.Binary(string="Upload File")

    def import_general_warehouse_group(self):
        try:
        ...
        try:
            ...
            for i in range(1, sheet.nrows):
                ...
                general_warehouse_group_id = self.env['general.warehouse.group'].browse(self._context.get('active_id'))
                warehouse_ids = []
                for i in range(1, sheet.nrows):
                    warehouse_id = self.env['stock.warehouse'].search([('code', '=', sheet.cell_value(i, 0))], limit = 1)
                    if warehouse_id:
                ...
                general_warehouse_group_id.warehouse_ids = list_warehouse
