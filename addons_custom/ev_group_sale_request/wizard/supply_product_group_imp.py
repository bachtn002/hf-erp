...
class SupplyProductGroupIMP(models.TransientModel):
    _name = 'supply.product.group.import'

    upload_file = fields.Binary(string="Upload File")

    def import_supply_product_group(self):
        try:
        ...
        try:
            ...
            if mess_error:
                ...
                general_product_group_id = self.env['supply.product.group'].browse(self._context.get('active_id'))
                product_ids = []
                for i in range(1, sheet.nrows):
                    product_id = self.env['product.product'].search(['|', ('default_code', '=', sheet.cell_value(i, 0)),
                                                                     ('barcode', '=', sheet.cell_value(i, 0))], limit=1)
