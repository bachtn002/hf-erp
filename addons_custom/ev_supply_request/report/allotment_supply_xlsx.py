...
class ExportSupplyRequest(models.AbstractModel):
    _name = 'report.ev_supply_request.allotment_supply_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        for obj in lines:
            ...
            sheet.write('C5', 'Mã sản phẩm:', format2)
            sheet.write('D5', 'Tên sản phẩm', format2)
            sheet.write('E5', 'Đơn vị đặt hàng(ĐVT)', format2)
            sheet.write('F5', 'Mã NCC', format2)
            sheet.write('G5', 'Số lượng yêu cầu', format2)
            sheet.write('H5', 'Đơn giá', format2)
            ...
            for line in obj.line_ids:
                ...
                sheet.write('C' + str(index), line.product_id.default_code, format6)
                sheet.write('D' + str(index), line.product_id.name, format6)
                sheet.write('E' + str(index), line.product_id.uom_po_id.name, format6)
                sheet.write('F' + str(index), line.partner_id.ref if line.partner_id else '', format6)
                sheet.write('G' + str(index), line.qty_request, format5)
                sheet.write('H' + str(index), line.price_unit, format6)
            ...
            sheet.merge_range('F' + str(index + 3) + ':' + 'H' + str(index + 3), 'Lập phiếu',
                              format2)
            sheet.conditional_format(0, 0, index, 8,
