...
class FixedAssetTrackingMonthReport(models.TransientModel):
    _name = "fixed.asset.tracking.month.report"
    _inherit = 'report.base'
    _auto = True
    _description = _("Fixed Asset Tracking Month Report")

    def _get_default_template_view_external_id(self):
        return 'ev_report_custom_v1.fixed_asset_tracking_month_report_template'
    ...
    def get_download_file_name(self):
        return _('fixed_asset_tracking_month_report.xlsx')

    def _get_report_json(self):
        params = self.copy_data()[0]
        Store = self.env['fixed.asset.tracking.month.report.store']

        return Store.get_data(**params)

    def _build_report_pandas_pivot(self, data):
        pv = super(FixedAssetTrackingMonthReport,
