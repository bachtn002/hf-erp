...
class StockMoveInOutReportStore(models.Model):
    _name = 'stock.move.in.out.report.store'
    _description = 'Stock Move In Out Report Store'
    _auto = False
	...
	(SELECT location_name,  product_code, product_name, dv,
	 ...
	 SUM(nhapkho) nhapkho, 
	 SUM(xuatkho) xuatkho, 
	 SUM(tondau)- SUM(xuatkho)+ SUM(nhapkho) toncuoi
	FROM (
	-- tồn kho đầu kỳ
...
WHERE (toncuoi != 0 OR tondau != 0 OR nhapkho != 0 OR xuatkho != 0)
        """
