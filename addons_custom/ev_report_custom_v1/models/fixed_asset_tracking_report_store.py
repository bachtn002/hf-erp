...
class FixedAssetTrackingReportStore(models.Model):
    _name = 'fixed.asset.tracking.report.store'
    _description = 'Fixed Asset Tracking Report Store'
    _auto = False
    ...
    def _get_data_query(self, **kwargs):
        ...
        try:
            ...
            sql = """
            SELECT COAlESCE(mats,'') mats, COAlESCE(tents,'') tents, 
                COAlESCE(trangthai, '') trangthai, 
                COAlESCE(nguyengia, 0) nguyengia, 
                COAlESCE(gtbatdaukh, 0) gtbatdaukh, 
                COAlESCE(gtdakh, 0) gtdakh,
                COAlESCE(gtkhtrongky, 0) gtkhtrongky, 
                COAlESCE(gtconlai, 0) gtconlai, 
                COAlESCE(gttbtrongky, 0) gttbtrongky, 
                COAlESCE(CAST(ngaymua AS VARCHAR), '') ngaymua, 
                COAlESCE(CAST(ngaytinhkh AS VARCHAR), '') ngaytinhkh, 
                COAlESCE(CAST(ngaygiam AS VARCHAR), '') ngaygiam, 
                COAlESCE(sokyhk, 0) sokyhk,
                COAlESCE(CAST(ngayct AS VARCHAR), '') ngayct, 
                COAlESCE(soct, '') soct, COAlESCE(tktaisan, '') tktaisan, 
                COAlESCE(tkkhauhao, '') tkkhauhao, COAlESCE(tkchiphi, '') tkchiphi,
                COAlESCE(mabp, '') mabp, COAlESCE(maphi, '') maphi, 
                COAlESCE(tenphi, '') tenphi, COAlESCE(tenbp, '') tenbp
            from 
            (
            select asset_tracking_b.id, asset_tracking_b.tents, asset_tracking_b.
                mats, asset_tracking_b.nguyengia, asset_tracking_b.gtbatdaukh,
                case
            ...
            from 
                (select id, tents, mats, nguyengia, gtbatdaukh, gtdakh,
                    sum(gtkhtrongky) gtkhtrongky, 0 as gtconlai,
                    sum(gttbtrongky) gttbtrongky, ngaymua, ngaytinhkh,
                    ngaygiam, sokyhk,
        --          ngayct,
        --          soct,
                    tktaisan, tkkhauhao, tkchiphi, mabp, maphi, tenphi,
                    tenbp, trangthai
            from 
                (SELECT b.id id, b.name tents, b.x_code mats,
                    ...
                    sum(a.amount_total) gtkhtrongky,0 gtconlai, 
                    avg(a.amount_total) gttbtrongky,
                    to_date(to_char(b.acquisition_date + INTERVAL '7 hours', 'dd/mm/yyyy'),'dd/mm/yyyy') ngaymua,
                    ...
                    case
                        when (select to_date(to_char(am.date + INTERVAL '7 hours', 'dd/mm/yyyy'), 'dd/mm/yyyy')
                            ...
                            left join account_account amlaa 
                            on amlaa.id = aml.account_id
                    ...
                    then (select to_date(to_char(am.date + INTERVAL '7 hours', 'dd/mm/yyyy'),'dd/mm/yyyy')
                        ...
                        and am.state = 'posted'
                        and aa.state = 'close'
                        order by am.write_date desc limit 1) end as ngaygiam,
                    b.method_number sokyhk, c.code tktaisan, d.code tkkhauhao,
                    e.code tkchiphi, f.code mabp, g.code maphi, g.name tenphi,
                    f.name tenbp, b.state trangthai
                from account_move a
                ...
                left join account_expense_item g 
                on g.id = b.x_account_expense_item_id
                ...
                and to_date(to_char(a.date + INTERVAL '7 hours', 'dd/mm/yyyy'), 'dd/mm/yyyy') <= to_date(%s, 'dd/mm/yyyy')
                group by b.id, tents, mats, nguyengia, gtbatdaukh, gtdakh, gtconlai,
                    ngaymua, ngaytinhkh, ngaygiam, sokyhk, tktaisan, tkkhauhao,
                    tkchiphi, mabp, maphi, tenphi, tenbp, trangthai
                ...
                SELECT b.id id, b.name tents, b.x_code mats, 
                    ...
                    0 gtkhtrongky, 0 gtconlai, 0 gttbtrongky,
                    to_date(to_char(b.acquisition_date + INTERVAL '7 hours', 'dd/mm/yyyy'),'dd/mm/yyyy') ngaymua,
                    to_date(to_char(b.first_depreciation_date + INTERVAL '7 hours', 'dd/mm/yyyy'),'dd/mm/yyyy') ngaytinhkh,
                    case
                        when (select to_date(to_char(am.date + INTERVAL '7 hours', 'dd/mm/yyyy'), 'dd/mm/yyyy')
                        ...
                        then (select to_date( to_char(am.date + INTERVAL '7 hours', 'dd/mm/yyyy'),'dd/mm/yyyy')
                            ...
                            and am.state = 'posted'
                            and aa.state = 'close'
                            order by am.write_date desc limit 1) 
                    end as ngaygiam,
                    b.method_number sokyhk, c.code tktaisan, d.code tkkhauhao,
                    e.code tkchiphi, f.code mabp, g.code maphi, g.name tenphi,
                    f.name tenbp, b.state trangthai
                ...
                and b.state != 'model'
    --          and b.book_value = 0
                and to_date(to_char(b.acquisition_date + INTERVAL '7 hours', 'dd/mm/yyyy'), 'dd/mm/yyyy') >= to_date(%s, 'dd/mm/yyyy')
                and to_date(to_char(b.acquisition_date + INTERVAL '7 hours', 'dd/mm/yyyy'), 'dd/mm/yyyy') <= to_date(%s, 'dd/mm/yyyy')
                group by b.id, tents, mats, nguyengia, gtbatdaukh, gtdakh,
                    gtconlai, gtkhtrongky, ngaymua, ngaytinhkh, ngaygiam, sokyhk,
                    tktaisan, tkkhauhao, tkchiphi, mabp, maphi, tenphi, tenbp
               ) as asset_tracking
                group by id, tents, mats, nguyengia, ngaymua, gtbatdaukh, gtdakh,  
                    ngaytinhkh, ngaygiam, sokyhk, tktaisan, tkkhauhao, tkchiphi,
