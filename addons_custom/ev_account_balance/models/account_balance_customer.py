...
class AccountBalanceCustomer(models.Model):
    _name = 'account.balance.customer'
    ...
    _sql_constraints = [
        ('account_balance_customer', 'unique (partner_id, account_id)',
         'You can not have two record with the same account and customer!'),
    ]
    ...
    def create(self, vals):
        id = super(AccountBalanceCustomer, self).create(vals)
        ...
        journal_id = self.env['account.journal'].search([('type', '=', 'sale'),('company_id','=', id.company_id.id)], limit=1)
        if not journal_id:
            raise except_orm(_('Thông báo'),
                             _('Bạn chưa cấu hình sổ nhật ký Công nợ khách hàng!'))
        move_lines = []
        if id.debit > 0:
            debit_move_vals = {
                'name': id.name,
            ...
            if vals.get('debit') > 0:
                if self.invoice_id:
                    sql = """
                        ...
                        USING account_balance_customer b 
                        WHERE 
                        b.invoice_id = a.id 
                        and b.id = %d;
                        """
        ...
        if 'credit' in vals.keys():
            if vals.get('credit') > 0:
                ...
                sql = """
                    ...
                    """
                self._cr.execute(sql % (account.id, move_id.id))
        return super(AccountBalanceCustomer, self).write(vals)
    ...
    def unlink(self):
        ...
        return super(AccountBalanceCustomer, self).unlink()