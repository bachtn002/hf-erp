...
class AccountBalanceAnalytic(models.Model):
    _name = 'account.balance.analytic'
    ...
    _sql_constraints = [
        ('account_balance_x_uniq', 'unique(account_id,analytic_account_id)', 'Account balance is unique')
    ]

    @api.model
    def create(self, vals):
        id = super(AccountBalanceAnalytic, self).create(vals)
        ...
        and a.date <= '%s'
        and NOT EXISTS (SELECT 1 FROM account_balance c WHERE a.move_id = c.move_id);"""
        self._cr.execute(query % (id.account_id.id, id.analytic_account_id.id, id.date, id.account_id.id,
                                  id.analytic_account_id.id, id.date))
        move_lines = []
        ...
        if id.debit > 0:
            debit_move_vals = {
                ...
                'debit': id.debit,
                'credit': 0.0,
                'analytic_account_id': id.analytic_account_id.id
            }
    ...
    def write(self, vals):
        ...
        if 'analytic_account_id' in vals.keys():
            sql = """
                    update account_move_line set analytic_account_id = %d where move_id = %d;
                """
            self._cr.execute(sql % (vals.get('analytic_account_id'), self.move_id.id))
        return super(AccountBalanceAnalytic, self).write(vals)
    ...
    def unlink(self):
        for line in self:
            if line.move_id:
                ...
                move_id.unlink()
        return super(AccountBalanceAnalytic, self).unlink()
