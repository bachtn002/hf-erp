...
class BangTinhCCDC(models.TransientModel):
    _name = "bang.tinh.ccdc"
...
class BangTinhCCDCWizard(models.TransientModel):
    _name = "bang.tinh.ccdc.wizard"
...
group by id, tents, mats, nguyengia, gtbatdaukh, ngaytinhkh, sokyhk, gtdakh, trangthai
        ...
        self.env.cr.commit()
        return {
            'name': _('Spreadsheet tools'),
            'view_mode': 'tree',
            'res_model': 'bang.tinh.ccdc',
            'domain': [('report_id', '=', report_id)],
            'res_id': self.id,
            'type': 'ir.actions.act_window',
            'search_view_id': self.env.ref('ev_report_view.bang_tinh_ccdc_search').id
        }
